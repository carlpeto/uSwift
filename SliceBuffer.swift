    ...
    RandomAccessCollection
{
...
#else
  @usableFromInline
  internal typealias NativeBuffer = _AVRArrayBuffer<Element>
#endif  
  ...
  internal var nativeBuffer: NativeBuffer {
    ...
    #endif
    #else
    return NativeBuffer(UnsafeMutableBufferPointer<Element>(start: subscriptBaseAddress, count: count))
    #endif
  }
...
// not sure if this is appropriate on array slice of fixed sized array
// my instinct is it should be a noop that returns the underlying buffer or similar

  @available(*, unavailable, message: "this should never be used")
  @inlinable
  ...
  internal func _checkValidSubscript(_ index : inout Int) {
    if index < startIndex || endIndex == 0 {
      ...
      _microswift_array_out_of_bounds(&index,startIndex,endIndex);
    ...
    let error = (index < startIndex || index >= endIndex)
  ...
  internal subscript(position: Int) -> Element {
    ...
    nonmutating set {
      _internalInvariant(position >= startIndex)
      _internalInvariant(position < endIndex)
