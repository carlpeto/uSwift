...
// these aborts put out reasonable console messages in full fat
// swift, but we don't have any kind of console to do that with
// so we are allowing an abort here, to provide defined behaviour
// probably an indicator stub would be good
void swift::swift_abortRetainUnowned(const void *object) {
  runtime_abort_function();
}

void swift::swift_abortRetainOverflow(const void *object) {
  runtime_abort_function();
}
...
#warning "The runtime must be built with a compiler that supports swiftcall. If not, check the consequences."
#endif
...
/* object (class instance) allocation */
...
// not doing all the ptrauth stuff on this platform...
...
// Callback for swift_once.
void initStaticObjectWithContext(void *OpaqueCtx) {
  InitStaticObjectContext *Ctx = (InitStaticObjectContext *)OpaqueCtx;
  Ctx->object->metadata = Ctx->metadata;
  Ctx->object->refCounts.initImmortal();
}
...
swift::swift_initStaticObject(HeapMetadata const *metadata,
  ...
  // short circuit, make it single threaded
  initStaticObjectWithContext(&Ctx);
...
#warning "destroyGenericBox parameter should be SWIFT_CONTEXT but swiftcall is not supported on this platform, investigate any call site issues"
/// Heap object destructor for a generic box allocated with swift_allocBox.
static SWIFT_CC(swift) void destroyGenericBox(HeapObject *o) {
...
BoxPair swift::swift_allocBox(const Metadata *type) {
  ...
  // to avoid pulling in lots of dependencies, we are not using the cache
  // auto metadata = &Boxes.getOrInsert(type).first->Data;
  auto newBox = new BoxCacheEntry(type);
  auto metadata = &newBox->Data;
