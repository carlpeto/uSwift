...
private:
  enum : int_type {
    UnusedLowBits = 0x07UL,      // historical conformance kind

    TypeMetadataKindMask = 0x7UL << 3, // 8 type reference kinds
    TypeMetadataKindShift = 3UL,

    IsRetroactiveMask = 0x01UL << 6,
    IsSynthesizedNonUniqueMask = 0x01UL << 7,

    NumConditionalRequirementsMask = 0xFFUL << 8,
    NumConditionalRequirementsShift = 8UL,

    HasResilientWitnessesMask = 0x01UL << 16,
    HasGenericWitnessTableMask = 0x01UL << 17,
  };
